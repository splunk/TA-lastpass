...
def validate_input(helper, definition):
    ...
    try:
        ...
        helper.log_error('%s: LastPass report input failed. %s' % (e.__class__.__name__, e.message))
        return None
...
def get_time_dt(helper, time_val):
    ...
    try:
        ...
        helper.warning('Validating time format. Time conversion failed. [time_val="%s"] reason="%s"' % (time_val, str(e)))
        return None

    diff = datetime.datetime.now() - time_dt
    # check for within last 4y or if time is ahead
    if diff.days > (365*4) or diff.days < 0:
        helper.log_warning('Validating time format. out of range. time val: "%s"' % time_val)
        return None
...
def save_checkpoint(helper, time_val):
    ...
    try:
        ...
        raise IOError('Save checkpoint failed. [time_val=%f] reason="%r"' % (time_val, repr(e)))
...
def get_checkpoint(helper):
    ...
    try:
        ...
        helper.log_warning('Loading checkpoint. Unable to load checkpoint. reason="%r"' % repr(e)) 
        return None

    if str(time_val).isdigit():
        return time_val

    helper.log_warning('Loading checkpoint. Checkpoint time value not in epoch time. [time_val="%r"]' % repr(time_val))
    return None
...
def collect_events(helper, ew):
    ...
    helper.log_debug('LastPass parameter check: rest_url=%s time_checkpoint=%s time_start="%s"' % (rest_url, time_checkpoint, time_start))
    headers = {}
    ...
    if day_incr > 1:
        helper.log_info('Lastpass report collection. large date range. start: %s\tend: %s' % (time_start, time_now))

    for i in xrange(0, diff.days+1, day_incr):
        ...
        try:
            ...
            for ev_id in resp_ev_json['data']:
                ...
                event_time = convert_time(ev_payload['Time']).strftime('%s')
                ...
                if chk_ptr % 1000 == 0:
                    ...
                    helper.log_debug('Updating checkpoint to date: %s' % event_time)
