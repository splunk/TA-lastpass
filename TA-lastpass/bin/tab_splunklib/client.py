...
"""The **tab_splunklib.client** module provides a Pythonic interface to the
...
**tab_splunklib.client** wraps a Pythonic layer around the wire-level
binding of the **tab_splunklib.binding** module. The core of the library is the
...
with the :func:`connect` function::
    ...
    import tab_splunklib.client as client
...
class Service(_BaseService):
    ...
    :class:`tab_splunklib.binding.Context` are also available in case you need
...
class Entity(Endpoint):
    ...
    def access(self):
        ...
        :return: A :class:`tab_splunklib.data.Record` object with three keys:
...
class ReadOnlyCollection(Endpoint):
    ...
    def itemmeta(self):
        ...
        :return: A :class:`tab_splunklib.data.Record` object containing the metadata.
...
class Collection(ReadOnlyCollection):
    ...
    **Example**::
    ...
    def create(self, name, **params):
        ...
        :type name: ``string``
        :param namespace: A namespace, as created by the :func:`tab_splunklib.binding.namespace`
            function (optional).  You can also set ``owner``, ``app``, and
            ``sharing`` in ``params``.
        :type namespace: A :class:`tab_splunklib.data.Record` object with keys ``owner``, ``app``,
            and ``sharing``.
...
class Inputs(Collection):
    ...
    def itemmeta(self, kind):
        ...
        :return: The metadata.
        :rtype: class:``tab_splunklib.data.Record``
        """
...
class Job(Entity):
    ...
    def results(self, **query_params):
        ...
        nice, Pythonic iterator, pass the handle to :class:`tab_splunklib.results.ResultsReader`,
        as in::

            import tab_splunklib.client as client
            import tab_splunklib.results as results
            from time import sleep
            service = client.connect(...)
    ...
    def preview(self, **query_params):
        ...
        Unlike :class:`tab_splunklib.results.ResultsReader`, which requires a job to
        be finished to
        ...
        the handle returned to :class:`tab_splunklib.results.ResultsReader` to get a
...
class Jobs(Collection):
    ...
    def export(self, query, **params):
        ...
        pass the handle to :class:`tab_splunklib.results.ResultsReader`::
